...
The roster is the five **strongest** decks from the portfolio — three Spark of Rebellion (Sabine, Boba, Leia) and two Ashes of the Empire (Cad Bane, Luke, the two leaders the early ASH community rated highest) — rebuilt **set-pure**: the three SOR decks contain only SOR cards and the two ASH decks only ASH cards, leader and base included. Because three of them are Rebel/Heroism go-wide decks that draw from the same limited staples, the two non-overlapping decks stay closest to their original builds while the three Rebel decks each swap roughly a dozen cards for their own on-aspect substitutes; that divergence is the unavoidable cost of physically separating decks that share a card pool. The two mono-ASH decks are deliberate homebrews — early community ASH shells lean heavily on SOR staples, so set purity means forging ahead without them until the collection grows. Premium printings (Standard Foil, Hyperspace, Hyperspace Foil, Prestige) are assigned first, then Standard, so the fanciest copies land in the boxes.
