...
The **Travel Decks** tab holds a curated roster of ten decks meant to be sleeved into ten physical deck boxes at once. Unlike the main Decks tab — where every deck is independently valid but they overlap on shared staples — the travel roster is a **disjoint allocation**: every physical card, leader, and base is assigned to at most one deck, never exceeding the owned quantity. Build all ten simultaneously and no copy is double-used.

The original five boxes remain fixed and set-pure: three Spark of Rebellion decks (Sabine, Boba, Leia) and two Ashes of the Empire decks (Cad Bane, Luke). The sixth, **Thrawn's Return Protocol**, is a mixed SOR/ASH Sloane Aggression deck built only from cards left after those five allocations. Its Imperial unit density supports Sloane's arena-wide Sentinel and Overwhelm choice, while Superlaser Technician and Resupply help reach the Prestige Foil Grand Admiral Thrawn. The seventh, **Gilded Nevarro Circuit**, is a Greef Karga Vigilance deck built from what remains after all six earlier boxes. It deliberately showcases every compatible remaining premium printing while retaining a 36-unit midrange shell: 10 Standard Foils, 13 Hyperspace cards, 3 Hyperspace Foils, plus a Hyperspace leader and base. It also uses all seven compatible Legendary/Special copies.

The final three boxes consume the maximum remaining ASH pool that can be played without aspect penalties, then backfill from SOR: **Charming Last Stand** is a Grogu Vigilance Sentinel/Ambush deck (18 ASH, 32 SOR); **Initiative Is the Way** is a Mandalorian Command action-compression deck (12 ASH, 38 SOR); and **The Emperor's Long Game** is a Palpatine Command Advantage swarm (18 ASH, 32 SOR). Together they use 48 ASH and 102 SOR main-deck cards. The only unused ASH main-deck card is Outcast, whose Aggression/Cunning icons cannot fit any of the three chosen aspect combinations without a penalty. Every card in all ten boxes is on-aspect.

The tab shows a live "all ten build at once" check against the current collection, each 50-card list with cost curve, owned coverage, and premium-printing chips, plus a SWUDB export per box. Roster data lives in `site/src/data/travel-decks.json` and is generated by `scripts/build-travel-roster.mjs` (re-run it after collection updates; it substitutes and reallocates deterministically); `scripts/validate-travel.mjs` and `site/src/lib/travel.test.ts` enforce that each deck is a legal 50 with zero aspect penalties and no more than three copies per card title, that leaders are distinct, and that the whole roster fits inside the owned collection at both the card and the individual-printing level (two boxes never claim the same single foil). Owning several copies of a base lets two boxes share a base identity.
...
The refresh script validates and reapplies `data/card-overrides.json` after downloading card metadata. It supports corrected names, subtitles, types, costs, power, and HP. Add an override only after checking the printed card; collection set/number mistakes should instead be corrected in the private HoloScan export.
...
- Confirmed upstream card-metadata errors are kept in `data/card-overrides.json` and reapplied by every refresh; collection exports never need to imitate a public API error.
